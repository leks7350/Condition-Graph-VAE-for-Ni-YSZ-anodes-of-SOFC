...
Данный проект предлагает пайплайн для анализа микроструктурных изменений в анодах твердооксидного топливного элемента (ТОТЭ, SOFC). Мы преобразуем 3D-томограммы в графовые представления и обучаем условный вариационный автоэнкодер на графах (`Conditional Graph VAE`), что позволяет эффективно выявлять скрытые признаки деградации и сравнивать исходные и деградировавшие состояния.
...
- сравнить эффективность различных ML моделей выявлять скрытые признаки и различия между структурами `Ni-YSZ` до и после деградации (воздействия окислительно-восстановительных циклов, редокс-циклов)
- разработать полный пайплайн обучения итоговой ML-модели
...
Сверточные слои `Conv2d` эффективно улавливают локальные паттерны (грани, текстуры, формы) двумерной структуры изображений. 
...
# 3. Построение графов по трехфазной структуре анода `Ni-YSZ` и обучение Conditional Graph VAE
...
**Свойство 3: Топологическая двойственность**
...
## Визуализация образца графов (образец pristine_fine_16.npy):
...
# 3.2. Обучение Conditional Graph VAE на построенных графах
...
<img width="4500" height="1800" alt="latent_space_analysis (1)" src="https://github.com/user-attachments/assets/cfee29df-bd4c-4c53-9b29-c504a74a52cc" />
...
2. Предсказание устойчивости новых структур к деградации (наиболее удаленное от зоны `degraded`)
3. Имитация деградации после различных циклов включения (редокс-циклов)
